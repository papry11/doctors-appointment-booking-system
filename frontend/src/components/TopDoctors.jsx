...
const TopDoctors = () => {
  ...
  return (
    <div className='flex flex-col items-center gap-6 my-15 text-gray-900 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 '>
        {doctors.slice(0, 10).map((item, index) => (
          <div onClick={()=>navigate(`/appointment/${item._id}`)}
            key={index}
            className='rounded-xl border border-teal-100 bg-blue-50 overflow-hidden shadow-sm hover:translate-y-[-10px] transition-all duration-500'
          >
            <img
              src={item.image}
              alt={item.name}
      ...
      <button onClick={() => { navigate('/doctors'); scrollTo(0,0)}} className='mt-10 px-12 py-3 bg-blue-100 text-gray-800 rounded-full border border-gray-300 hover:bg-blue-200 cursor-pointer'>
        More
      </button>
    </div>
  )
